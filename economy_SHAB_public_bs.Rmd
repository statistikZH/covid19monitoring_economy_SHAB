...
p_ch_cum <- ggplot(pldf_cum,aes(day_date,cum_chsum,col=year,group=year)) +
  geom_path(aes(text=sprintf("Tag: %s<br>Kumulierte Tageswerte: %s<br>Tageswert: %s", date,cum_chsum,chsum)))+
...
p_ch <- ggplot(pldf,aes(day_date,rollmean,col=year,group=year)) +
  geom_path(aes(text=sprintf("Tag: %s<br>5-Tage-Mittelwert: %s<br>Tageswert: %s", date,rollmean,chsum)))+
      scale_color_manual(values = c('2019'='#EDDD79',
                                    '2020'='#F3A5A7')) +
  scale_x_date(labels = date_format("%d.%m"))+
...
geom_path(aes(col=year, group=year,
              text=sprintf("Datum: %s<br>Kumulierter Wert: %s<br>Tageswert: %s", date,cum_cant,value)))+
  scale_color_manual(values = c('2019'='#EDDD79',
                                    '2020'='#F3A5A7')) +
...
pn_ggplot <- paste0('~/covid19monitoring_server/economy_SHAB/cant_ggplots/',i,'.png')
ggsave(p_cant_cum, file =pn_ggplot)
...
pn <- paste0('~/covid19monitoring_server/economy_SHAB/cant_plotlies/',i,'.html')
htmlwidgets::saveWidget(as_widget(gg_cant_cum), file =pn)

}

cant_plots <- paste0('~/covid19monitoring_server/economy_SHAB/',list.files("cant_ggplots", full.names = TRUE))
names(cant_plots) <- stringr::str_replace_all(state_plots,
                                      c("\\.png" = "", 
                                        "cant_ggplots/" = "",
                                         '~/covid19monitoring_server/economy_SHAB/'= ''))

bsselect(cant_plots, type = "img", selected = "AG", 
         live_search = TRUE, show_tick = TRUE)

htmlwidgets::saveWidget(as_widget(cant_widget), file ='cant_widget.html')


bsselectR::as_iframe(cant_widget, file = NULL, height = "500", width = "100%")
...
bsselectR::bsselect(quotes, type = "text")
```
...
geom_path(aes(col=year, group=year,
              text=sprintf("Tag: %s<br>5-Tage-Mittelwert: %s<br>Standardisierter Mittelwert: %s<br>Tageswert: %s", date,rollmean,rollmean_zvalue,value)))+
  scale_color_manual(values = c('2019'='#EDDD79',
                                    '2020'='#F3A5A7')) +
  scale_x_date(labels = date_format("%d.%m"), breaks = as.Date(c('01.03','01.06','01.09','01.12'),format="%d.%m"))+
  facet_wrap(~ location,ncol=3)+
